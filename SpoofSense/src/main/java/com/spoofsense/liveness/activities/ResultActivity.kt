...
internal class ResultActivity : AppCompatActivity() {
    ...
    private fun initViews(){
        binding.apply{
            val filter = BlendModeColorFilterCompat.createBlendModeColorFilterCompat(SpoofSense.buttonBackgroundColor, BlendModeCompat.SRC_ATOP)
    ...
    private fun initAntiSpoof(){
        intent.getStringExtra(IntentConstant.DATA)?.let { data->
            antiSpoofVM.antiSpoof(data){response->
                binding.apply {
                    if(response?.modelOutput?.isReal() == true){
                        ...
                        ivResult.setImageDrawable(ContextCompat.getDrawable(this@ResultActivity,R.drawable.success))
                        ...
                        tvResult.text = resultDO.message
                        ivResult.setImageDrawable(ContextCompat.getDrawable(this@ResultActivity,R.drawable.error))
    ...
    companion object{
        fun newInstance(context: Context,data:String): Intent{
            return Intent(context,ResultActivity::class.java)
                .putExtra(IntentConstant.DATA,data)
        }
    }
}